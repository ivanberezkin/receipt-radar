...
public interface ReceiptRepository extends JpaRepository<ReceiptEntity, Long> {
    ...
    @Query("SELECT new com.example.DTO.ChartDataDto(r.date, COALESCE(SUM(r.amountPaid), 0)) FROM ReceiptEntity r where r.date BETWEEN :start AND :end group by r.date order by r.date ASC")
